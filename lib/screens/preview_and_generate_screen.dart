  ...
  Future<void> _generateAllCertificates() async {
    ...
    try {
      ...
      for (int i = 0; i < allCertData.length; i++) {
        ...
        final pdfPath =
        p.join(outputDir.path, "${certData.certificateId}.pdf");
      ...
      // Only fetch userDoc after currentUser is verified
      final userDoc = await FirebaseFirestore.instance
          .collection('users')
          .doc(currentUser.uid)
          .get();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Failed to generate some certificates.")));
    } finally {
      setState(() => isGenerating = false);
    }
  }
